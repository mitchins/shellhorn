...
class ActiveCommand:
    ...
    def age_seconds(self) -> float:
        ...
        if age < 0:
            logger.warning(f"Negative age detected for command {self.command}: {age}s (start_time: {self.start_time}, now: {datetime.now()})")
        return age
...
class ShellhornMonitor:
    ...
    def setup_mqtt(self):
        ...
        client_id = self.config.get('client_id', f"shellhorn-monitor-{int(time.time())}")
        ...
        self.mqtt_client.will_set(f"{topic_prefix}/status", "offline", qos=1, retain=True)
    ...
    def handle_command_start(self, command_key: str, payload: Dict[str, Any]):
        ...
        logger.debug(f"Received timestamp: {timestamp_val} (type: {type(timestamp_val)})")
    ...
    def check_orphaned_commands(self):
        ...
        logger.debug(f"Checking for orphaned commands (timeout: {timeout_minutes} minutes)")
        
        with self.lock:
            orphaned = []
            for key, command in list(self.active_commands.items()):
                age_minutes = command.age_seconds / 60
                logger.debug(f"Checking command {command.command}: age={age_minutes:.2f}m, timeout={timeout_minutes}m")
        ...
        for command in orphaned:
            age_minutes = command.age_seconds / 60
            logger.warning(f"Orphaned command detected: {command.command} (age: {age_minutes:.1f} minutes, started: {command.start_time})")
            logger.info(f"Sending lost command notification for: {command.command}")
            self.send_event_notification(command, "lost")

    def send_event_notification(self, command: ActiveCommand, event_type: str, duration: float = None):
        ...
        logger.debug(f"Processing {event_type} notification for command: {command.command}")
        ...
        if not (pushover_token and pushover_user):
            logger.warning(f"Pushover credentials not configured (token: {bool(pushover_token)}, user: {bool(pushover_user)}), skipping notification")
        ...
        base_message = message_templates.get(event_type, f"Command {event_type}: {{command}}")
        ...
        try:
            self.send_pushover_alert(pushover_token, pushover_user,
                                   secrets.get('pushover_device'), message, priority, event_type)
            logger.info(f"Pushover {event_type} notification sent for command: {command.command}")
        except Exception as e:
    ...
    def send_pushover_alert(self, token: str, user: str, device: Optional[str], 
        ...
        if priority == 2:
            emergency_config = self.config.get('notifications', {}).get('pushover', {}).get('emergency', {})
            data['retry'] = emergency_config.get('retry', 60)        # Default: retry every 60 seconds
            data['expire'] = emergency_config.get('expire', 3600)    # Default: stop after 1 hour
    ...
    def status_report(self, log_only=False):
        ...
        with self.lock:
            ...
            if active_count > 0:
                ...
                for key, command in self.active_commands.items():
                    ...
                    logger.info(f"  - {command.command} (age: {age_minutes:.1f}m, started: {command.start_time}, age_sec: {age_seconds:.1f})")
            else:
    ...
    def publish_heartbeat(self):
        ...
        with self.lock:
            ...
            status_data = {
                ...
                'uptime_seconds': round(time.time() - self.start_time) if hasattr(self, 'start_time') else 0,
                'monitor_id': self.config.get('client_id', 'shellhorn-monitor'),
                'commands': []
            }
    ...
    def run(self):
        ...
        heartbeat_interval = self.config['monitoring'].get('heartbeat_interval_seconds', 60)
...
def load_config() -> Dict[str, Any]:
    ...
    default_config = {
        ...
        'monitoring': {
            ...
            'status_interval_seconds': int(os.getenv('SHELLHORN_STATUS_INTERVAL', '300')),
            'heartbeat_interval_seconds': int(os.getenv('SHELLHORN_HEARTBEAT_INTERVAL', '60'))
        },
        'notifications': {
            'enabled_events': {
                'start': False,
